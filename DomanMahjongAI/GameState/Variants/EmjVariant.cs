...
/// <summary>
/// Layout implementation for the addon seen on EU / most non-EmjL clients
/// (name <c>Emj</c>). Holds every hard-coded offset, node ID, and AtkValue
/// slot this addon uses — extracted verbatim from the original
/// <see cref="AddonEmjReader"/> so Sequence 1 can land with zero behavior
/// change. A future <c>EmjLVariant</c> will sit beside this class, not above
/// or inside it (sibling variants, not inheritance — see issue #13 thread).
/// </summary>
internal sealed class EmjVariant : IEmjVariant
{
    public string Name => "Emj";

    // Log de-dupe state for diagnostic dumps. Scoped to the variant so each
    ...
    /// <summary>
    /// Fingerprint check: the self-score word at +0x0500 is in the plausible
    /// mahjong range, and the call-modal host node (id=104) exists. Either
    /// check alone produces false positives on unrelated layouts; both
    /// together have held across every Emj capture to date.
    /// </summary>
    public unsafe bool Probe(AtkUnitBase* unit)
    {
        ...
        return unit->GetNodeById(104) != null;
    }
